...
class _StackedBarChartState extends State<StackedBarChart> {
  ...
  void initState() {
    ...
    chartData = <_ChartData>[
      _ChartData('탄수화물', 24, 100),
      _ChartData('단백질', 65, 100),
      _ChartData('지방', 51, 100),
      _ChartData('총 식이섬유', 24, 100),
      _ChartData('콜레스테롤', 48, 100),
      _ChartData('총 포화 지방산', 48, 100),
    ];
  ...
  SfCartesianChart _buildStackedBar100Chart() {
    return SfCartesianChart(
      ...
      title: ChartTitle(text: isCardView ? '' : 'kcal data'),
  ...
  List<ChartSeries<_ChartData, String>> _getStackedBarSeries() {
    return <ChartSeries<_ChartData, String>>[
      StackedBar100Series<_ChartData, String>(
          ...
          yValueMapper: (_ChartData sales, _) => sales.real,
          name: 'Real'),
      StackedBar100Series<_ChartData, String>(
          dataSource: chartData!,
          xValueMapper: (_ChartData sales, _) => sales.x,
          yValueMapper: (_ChartData sales, _) => sales.fiction,
          name: 'Fiction')
    ];
  }
...
class _ChartData {
  _ChartData(this.x, this.real, this.fiction);
  final String x;
  final num real;
  final num fiction;
}
